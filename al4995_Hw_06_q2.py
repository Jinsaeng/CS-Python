class Integer:
    ...
    def __add__(self, other):
        ...
        for i in range(len(greater)):
            digit = int(greater[end]) + int(lesser[end]) + digit
            next_digit = 0
            if digit > 10:
                digit -= 10
                next_digit += 1
            self.DoublyLinkedList.add_last(self.DoublyLinkedList.trailer.prev, digit)
            digit = next_digit
            end -= 1
    def __repr__ (self):
        return repr(DoublyLinkedList)
